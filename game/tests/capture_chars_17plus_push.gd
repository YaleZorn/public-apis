extends SceneTree
## Capture v0.9.7 17+ push media (stronger art + walk/attack frames).

const OUT := "/cursor/stores/bc-71787b67-91e4-456d-b541-da2778721eaf/media/chars-17plus-push"
...
func _shot_closeups() -> void:
	...
	for i in ids.size():
		...
		var fig: Control
		if str(ids[i]).begins_with("enemy_"):
			fig = VF.enemy_node(u if not u.is_empty() else {"id": ids[i], "name": "?", "color": "#a0522d", "tags": []}, Vector2(130, 170))
		else:
			fig = VF.unit_node(u if not u.is_empty() else {"id": ids[i], "name": "?", "role": "dps", "color": "#6a8f71"}, Vector2(130, 170))
