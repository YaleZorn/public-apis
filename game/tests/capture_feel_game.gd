extends SceneTree
## Capture v0.11 feel-like-a-game first session: title → lobby → guided TD.

const OUT := "/cursor/stores/bc-71787b67-91e4-456d-b541-da2778721eaf/media/feel-game"
...
func _run() -> void:
	...
	if gs:
		...
		gs.set("arena_best_sec", 0.0)
		gs._init_defaults()
		gs.meta_changed.emit()
		gs.checkpoint_changed.emit()
	await _shot("res://scenes/shell/title_screen.tscn", "01-title.png", 0.9)
	await _shot("res://scenes/lobby/lobby.tscn", "02-lobby-next.png", 1.0)
	await _shot_td_prep()
	# Fresh lobby after first-session unlock (no residual TD checkpoint).
	if gs:
		gs.set("td_checkpoint", {})
		gs.set("explore_checkpoint", {})
		gs.set("arena_checkpoint", {})
		...
		gs.meta_changed.emit()
		gs.checkpoint_changed.emit()
	await _shot("res://scenes/lobby/lobby.tscn", "04-lobby-unlocked.png", 1.0)
...
func _shot_td_prep() -> void:
	...
	# Start a short wave and wait for clear juice if possible
