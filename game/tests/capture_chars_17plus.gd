extends SceneTree
## Capture 17+ character presentation media.
...
func _shot_closeups() -> void:
	...
	var ids := ["unit_qinggong", "unit_mulan", "unit_feidao", "unit_zhaoyun", "unit_linchong", "unit_mingwang"]
	var x0 := 40.0
	for i in ids.size():
		var u: Dictionary = {}
		if cdb:
			u = cdb.get_unit(ids[i])
		var fig := VF.unit_node(u if not u.is_empty() else {"id": ids[i], "name": "?", "role": "dps", "color": "#6a8f71"}, Vector2(130, 170))
...
func _shot_portrait_grid() -> void:
	## Lobby-style portrait cards for review of 17+ collectible art.
	var host := Control.new()
	host.set_anchors_preset(Control.PRESET_FULL_RECT)
	host.size = Vector2(720, 1280)
	var bg := ColorRect.new()
