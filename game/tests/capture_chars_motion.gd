extends SceneTree
## Multi-frame capture of figure idle/attack for short video.
...
func _run() -> void:
	...
	var ids := ["unit_qinggong", "unit_mulan", "unit_feidao", "unit_tiebi"]
	var figs: Array = []
	for i in ids.size():
		...
		figs.append(fig)
	# Capture frames while bobbing; poke attack poses mid-way
	for f in 16:
		if f == 4 or f == 10:
			for fig in figs:
